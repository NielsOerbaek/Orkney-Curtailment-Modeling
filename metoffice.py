...
def evaluateMetForecast(start="2019-04-01",stop="2019-05-01", name="met-full-frame", code=0, load_partial=False, load_full=False):
    if load_full:
        if load_partial:
    ...
    pickle.dump(names, open(config.DATA_PATH+""+name+"-describes-name","wb"))

    return accs, names

def makeForecastAccTable(name="met-full-frame"):
    if True:
        accs = pickle.load(open(config.DATA_PATH+""+name+"-describes", "rb"))
        names = pickle.load(open(config.DATA_PATH+""+name+"-describes-name", "rb"))
...
def hitAccuracy():
    df = pickle.load(open(config.DATA_PATH+"met-full-frame-all","rb"))
...
def getFullCombinedMetFrame():
    df = pickle.load(open(config.DATA_PATH+"met-full-frame-all","rb"))
    df_clean = pickle.load(open(config.DATA_PATH+"met-full-frame-all-clean","rb"))
